...
def postDetail(request, permalink):
    ...
    if user != "undefined" and user != None:
        data["post_detail"]["upvoted"] = UpvotePost.objects.filter(
            post=post, user=user
        ).exists()
        data["post_detail"]["downvoted"] = DownvotePost.objects.filter(
            post=post, user=user
        ).exists()
        data["post_detail"]["saved"] = SavePost.objects.filter(
            post=post, user=user
        ).exists()
